...
namespace RaccoonNinja.McpToolset.Server.TextEdit.Tests.Configuration;
...
public sealed class ScopeResolverTests : IDisposable
{
    ...
    {
        // Act & Assert
        Assert.Throws<EditStartupException>(() => ScopeResolver.Create("", null, null));
    }
    ...
    {
        // Arrange
        var missing = Path.Combine(Path.GetTempPath(), $"rnmcp-te-missing-{Guid.NewGuid():N}");

        // Act & Assert
        Assert.Throws<EditStartupException>(() => ScopeResolver.Create(missing, null, null));
    }
    ...
    public void Create_BaseIsUserHome_FailsFast()
    {
        ...
        // Act & Assert
        Assert.Throws<EditStartupException>(() => ScopeResolver.Create(home, null, null));
    }
    ...
    public void Create_BaseIsFilesystemRoot_FailsFast()
    {
        ...
        // Act & Assert
        Assert.Throws<EditStartupException>(() => ScopeResolver.Create(root, null, null));
    }
    ...
    {
        // Arrange
        var parent = NewTempDirectory("reparent-leaf");
        var config = Directory.CreateDirectory(Path.Combine(parent, ".config")).FullName;

        // Act & Assert
        Assert.Throws<EditStartupException>(() => ScopeResolver.Create(config, null, null));
    }
    ...
    public void Create_BaseUnderDenylistedSegment_FailsFast()
    {
        ...
        // Act & Assert
        Assert.Throws<EditStartupException>(() => ScopeResolver.Create(under, null, null));
    }
    ...
    public void Create_BadExtraDeny_FailsFast()
    {
        ...
        // Act & Assert
        Assert.Throws<EditStartupException>(() => ScopeResolver.Create(baseRoot, null, "/etc/passwd"));
    }
    ...
    public void Create_DefaultIgnoreMissingFile_FailsFast()
    {
        ...
        // Act & Assert
        Assert.Throws<EditStartupException>(() => ScopeResolver.Create(baseRoot, missing, null));
    }
    ...
    public void Create_DefaultIgnoreFile_AppliesItsPatterns()
    {
        ...
        // Act
        var resolver = ScopeResolver.Create(baseRoot, ignoreFile, null);
    ...
    {
        // Arrange
        var resolver = ScopeResolver.Create(NewTempDirectory("whole"), null, null);
    ...
    public void Resolve_ValidSubdir_ScopeKeyIsRelative()
    {
        ...
        var resolver = ScopeResolver.Create(baseRoot, null, null);
    ...
    {
        // Arrange
        var resolver = ScopeResolver.Create(NewTempDirectory("in"), null, null);
        var outside = NewTempDirectory("out");

        // Act
        var exception = Assert.Throws<TextEditException>(() => resolver.Resolve(outside));
    ...
    public void Resolve_CwdIsFile_IsRefused()
    {
        ...
        // Act
        var exception = Assert.Throws<TextEditException>(() => resolver.Resolve(file));
    ...
    public void Resolve_CwdInsideDenylistedDir_IsRefused(string denied)
    {
        ...
        // Act
        var exception = Assert.Throws<TextEditException>(() => resolver.Resolve(deniedDir));
    ...
    public void Resolve_CwdLeafIsReparentUnsafe_IsRefused()
    {
        ...
        // Act
        var exception = Assert.Throws<TextEditException>(() => resolver.Resolve(configDir));
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), $"rnmcp-te-scope-{label}-{Guid.NewGuid():N}");
