...
- Changed release artifact naming. Per-platform zips are now `<platform>-<binary>-<version>.zip` (for example `win-x64-file-vault-1.0.0.zip`) instead of `<Project>-<version>-<rid>.zip`, and each platform additionally ships a `<platform>-AllTools-<version>.zip` bundle containing every tool. The `<binary>` token is each project's `AssemblyName` (the published executable name), resolved by the release workflow rather than taken from the project folder. Keying on the binary name lets the `skill-stats-mcp` server and `skill-stats-cli` hook keep distinct zips even though their projects now share the `SkillStats` name; a repeated binary name still fails the build. Anyone scripting downloads against the old names must adjust; existing releases are immutable and keep their old names.
...
- Changed `SkillUsage` namespace to `SkillStats` so the name matches between all binaries.
