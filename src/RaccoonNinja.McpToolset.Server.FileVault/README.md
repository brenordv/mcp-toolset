...
existing `vault` MCP registration at this binary and every note, version, and tag keeps working. The nine deliberate
...
| D9 | `vault_edit_section` matches a heading by either its rendered plain text (inline markdown stripped, the Rust behavior) or its verbatim source text with the delimiters kept. So a heading containing a code span, emphasis, or a link can be targeted with the text copied straight from the document. | Additive: the rendered form still matches, so a caller pasting a heading verbatim (backticks and all) no longer gets a spurious `heading_not_found`. In the rare case a verbatim form collides with another heading, the result is `ambiguous_heading`, never a wrong-section edit. |
