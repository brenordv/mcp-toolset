...
public sealed class FindFilesTool(ToolCommon common, SearchConfig config, ScopeResolver resolver)
{
    ...
    /// <param name="include_ignored">Globs that re-include otherwise-ignored paths (built-in default tier only, e.g. node_modules); never re-includes a project ignore-file path, and never bypasses the secret denylist.</param>
    ...
    public Task<ResultEnvelope> InvokeAsync(
        ...
        [Description("Globs that re-include otherwise-ignored paths for this call, e.g. [\"node_modules/**\"]. Re-includes only the built-in default tier; never re-includes a project ignore-file path, and never bypasses the secret denylist.")]
