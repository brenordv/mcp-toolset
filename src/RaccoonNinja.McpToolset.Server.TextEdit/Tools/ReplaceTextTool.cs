...
[McpServerToolType]
public sealed class ReplaceTextTool(ToolCommon common, EditConfig config, RootRegistry registry, GatedFileWriter writer)
{
    ...
    /// <param name="regex">A regex over root-relative paths selecting files, or null.</param>
    ...
    [Description(
        ...
        + "back-references ($1, ${name}, $$ for a literal $). Select with exactly one of glob, regex, or paths "
        + "(or none for the whole root), optionally narrowed by extensions. Set expected_match_count to abort "
        + "before any write unless exactly that many matches would change. Pass dry_run to preview a diff. "
        + "Every changed file is journaled for undo.")]
